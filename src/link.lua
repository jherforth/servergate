...
function servergate.link(pos)
  -- Get meta/destination
  local nodemeta = minetest.get_meta(pos)
  local destination = nodemeta:get("telemosaic:dest")
  local do_servergate_link = false

  -- Check for servergate destination and convert to vector
  local dpos = nil
  if not destination then
    dpos = nodemeta:get("servergate:destination")
    if not dpos then
      return true
    end
    destination = minetest.string_to_pos(dpos)
    do_servergate_link = true
  else
  ...
  for i = 1, #players do
    ...
    if player and player:get_pos():in_area(pos:add(-32),pos:add(32)) then
      if not nodemeta:get("servergate:player_nearby") then
        ...
        minetest.emerge_area(emin,emax,function(blockpos,action,calls_remaining)
          -- Do servergate link to Telemosaic if defined for this gate
          if calls_remaining == 0 then
            if do_servergate_link then
              destination = nil
              ...
              for beacon = 1, #beacons do
                ...
                local spos = minetest.get_meta(beacon):get("servergate:source")
                if dpos == spos then
                  destination = beacon
        ...
        nodemeta:set_string("servergate:player_nearby","1")
      end
      return true
    end
  end
...
telemosaic.teleport = function(player,src,dest)
  tt(player,src,dest)
  dest.y = dest.y - 1
  servergate.link(vector.new(src))
  servergate.link(vector.new(dest))
end
...
telemosaic.set_state = function(pos,state)
  tss(pos,state)
  servergate.link(vector.new(pos))
end

-- Register LBM to jump-start force-load timers
minetest.register_abm({
  label = "Run servergate linking function",
