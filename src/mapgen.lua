...
-- Spawning flags
local underwaterspawn = servergate.settings.underwaterspawn
local midairspawn = servergate.settings.midairspawn
...
local schematic_airspace = servergate.modpath .. "/schematics/servergate_airspace.mts"
local schematic_platform = servergate.modpath .. "/schematics/servergate_platform.mts"

local extender_break_chance = servergate.settings.breakage
...
minetest.register_on_generated(function(minp,maxp,blockseed)
  ...
  for x = 0, 4  do
    for y = 0, 4 do
      for z = 0, 4 do
        local hashed = servergate.get_gates_for_mapblock(vn(minp.x + x * 16,minp.y + y * 16,minp.z + z * 16))
        for h = 1, #hashed do
  ...
  for gate = 1, #gates do
    ...
    if gate.exact then
      ...
      if not location then
        ...
        for c = 1, #servergate.servergate_failed_callbacks do
          servergate.servergate_failed_callbacks[c](gate)
        end
        return -- cannot generate this servergate
      end
    ...
    for _,epos in ipairs(minetest.find_nodes_in_area(location:add(vn(-6,0,-6)),location:add(vn(6,3,6)),"group:servergate_extender")) do
      if pcgr:next(1,100) <= extender_break_chance then -- chance for any extender to be broken
        ...
        minetest.swap_node(epos,quality_selector[minetest.get_item_group(minetest.get_node(epos).name,"servergate_extender") + gate.quality](pcgr))
      end
    end
    ...
    -- Trigger callbacks
    for c = 1, #servergate.servergate_generated_callbacks do
      servergate.servergate_generated_callbacks[c](location,gate,strategy)
    end
  end
end)